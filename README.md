...
The twin keeps lawyer cards, hub service lists and sibling lists; it drops
TOC, upsell, forms and crumbs. Twin body links are absolute
(`https://noriet.pl/…`). `robots.txt` comments the `/llms.txt` URL; the
`Link` header still advertises it on every HTML response.
...
| `Article` | 141 posts | `h1`, `isoDate` as `datePublished`, `isoModified` or `isoDate` as `dateModified`, description, category label as `articleSection`; `publisher` is the firm Organization; `author` is the Person parsed from the migrated byline (`posts.11tydata.js` — team members linked by `@id`, former colleagues plain), the Organization where no byline exists |
